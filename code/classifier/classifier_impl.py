...
def train_text(text_src, convert_file = None, convert_arguments, grid_arguments):
    """Train Classifier based on text_src

    """
    name = path.split(text_src)[1]
    convert_file = svm_file or name + ".vec"
    
    text_converter = Text2vectConverter(convert_arguments)
    feats, label = convert_text(text_src, text_converter, convert_file)

def train_formatted_text(feats, labels, train_arguments = ''):
